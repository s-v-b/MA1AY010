...
format:
  pdf:
    ...
    class: exam
    include-in-header:
      - text: "\\lhead{{\\sf  Probabilités \\\\ TD 1}}"
  html:
...
```{r}
#| eval: false    # true si l'exercice est visible 
#| echo: false
cnt_exo <-  cnt_exo + 1
```
...
1. Lesquels parmi les $8$ graphes dessinés sur la dernière page représentent des fonctions de répartition? Justifier. 
...
1. Ce n'est pas une fonction de répartiton, car elle n'est pas croissante : $F(0-) > F(0+)$
1. C'est la fonction de répartition d'une variable $X$ telle que $\mathbb{P}(X=0)=p$, et $\mathbb{P}(X \in [0,1/2]) = 1$. Comme $\mathbb{P}(X=0)>0$, $X$ ne possède pas de densité. Enfin on a clairement $\mathbb{E}[X] \in (0,1/2)$. 
1. C'est une fonction de répartition, en admettant que $F(x) \to 1$ lorsque $x \to \infty$. 
...
```
 ...
 (Minimum, maximum de variables indépendantes)
...
1. Dans ce cas $F_{X_i}(x) = x \mathbf{1}_{[0,1]}(x) + \mathbf{1}_{]1,+\infty[}(x)$, et donc 
 
    $$F_{Z_k}(x) = {\begin{cases} 0 & \mbox{si } x \le 0\\ x^k & \mbox{si } 0 \le x \le 1 \\  1 & \mbox{si } x \ge 1 \end{cases},$$
...
|                                  |                             | $\approx$                     |
|:---------------------------------|:---------------------------:|------------------------------:|
|$\mathbb{P}(X \le 1)$             | `pnorm(1)`                  | `r pnorm(1)`                  |
|$\mathbb{P}(-1.23 \le X \le 0.43)$| `pnorm(.43) - pnorm(-1.23)` | `r pnorm(.43) - pnorm(-1.23)` |
|$\mathbb{P}(X > .32 1)$           | `1 - pnorm(.32)`            | `r 1- pnorm(.32)`             |
...
$$\begin{eqnarray*} \mathbb{P}(Z \le x) & = & \mathbb{P}(\xi=1,  X \le x) + \mathbb{P}(\xi=0, Y \le x) \\ & = & \mathbb{P}(\xi=1) \mathbb{P}(X \le x) + \mathbb{P}(\xi=0) \mathbb{P}(Y \le x) \\ & = & \alpha F_X(x) + (1-\alpha) F_Y(x) \\ & = & \int_{-\infty}^x (\alpha f_X(x) + (1-\alpha)f_Y(x)) dx \end{eqnarray*}$$
...
$$\begin{eqnarray*} \mathbb{E}[\phi(Y^2)] & = & \mathbb{E}[\phi(X^2)] \\ & = & \int_{\mathbb{R}} \phi(x^2) f(x) dx \\ & = & \int_{\mathbb{R}_-}\phi(x^2) f(x) dx + \int_{\mathbb{R}_+} \phi(x^2) f(x) dx \\ & = & \int_{0}^{\infty} \phi(u) f(-\sqrt{u}) \frac{du}{2\sqrt{u}} + \int_0^{\infty} \phi(u) f(\sqrt{u})\frac{du}{2\sqrt{u}} \\& = & \int_0^{\infty} \phi(u) \frac{f(-\sqrt{u}) + f(\sqrt{u})}{2\sqrt{u}} du \end{eqnarray*}$$
...
3. Soit $\phi : \mathbb{R} \to \mathbb{R}_+$ borélienne, on a, en effectuant le changement de variables $u=\exp(-x)$ à la troisième ligne ci-dessous 
    ...
    $$\begin{eqnarray*}
    \mathbb{E}[\phi(U)] & = & \mathbb{E}[\phi(\exp(-X)] \\ & = & 
    \int_0^{\infty} \phi(\exp(-x)) \exp(-x) dx \\ & = &  \int_{0}^1 \phi(u) du, 
    \end{eqnarray*}$$
...
1.  Soit $\phi : \mathbb{R} \to \mathbb{R}_+$ borélienne, on a, en effectuant le changement de variables $u=\arcsin(x)$ à la deuxième ligne ci-dessous
    ...
    $$\begin{eqnarray*}
     \mathbb{E}[\phi(\arcsin(X))] & = & \int_0^1 \phi(\arcsin(x)) dx  \\ & = & \int_0^{\pi/2} \phi(u) \cos(u) du. 
    \end{eqnarray*}$$
...
1. Soit $\phi : \mathbb{R} \to \mathbb{R}_+$ borélienne, on a, en effectuant le changement de variables $u=\arcsin(x)$ à la deuxième ligne ci-dessous
    ...
    $$\begin{eqnarray*}
    \mathbb{E}[\phi(\arcsin(X))] & = & \frac{1}{2} \int_{-1}^1 \phi(\arcsin(x)) dx  \\ & = & \frac{1}{2} \int_{-\pi/2}^{\pi/2} \phi(u) \cos(u) du. 
    \end{eqnarray*}$$
...
2. On a $h(\theta) = \tan(\theta)$, et donc si on pose $X = h(\theta)$ et si $\phi : \mathbb{R} \to \mathbb{R}_+$ est borélienne on a (on effectue le changement de variables $x=\tan(\theta)$ à la troisième ligne ci-dessous) 
    ...
    $$\begin{eqnarray*} \mathbb{E}[\phi(X)] & = & \mathbb{E}[\phi(\tan(\theta))] \\ & = & \int_{-\pi/2}^{\pi/2} \frac{2}{\pi} \phi(\tan(\theta)) d\theta \\ & = &  \int_{-\infty}^{\infty} \frac{1}{\pi} \frac{\phi(x)}{1+x^2} dx  \end{eqnarray*}$$
...
1.  Comme $\mathbb{P}(X=0)=0$, la variable $Y$ est bien définie. 
    ...
    $$\begin{eqnarray*} \mathbb{E}[\phi(Y)] & = & \mathbb{E}[\phi(\frac{1}{X})] \\ & = & \int_{-\infty}^{0} \frac{\phi(1/x)}{\pi(1+x^2)} dx  + \int_{0}^{\infty} \frac{\phi(1/x)}{\pi(1+x^2)} dx \\ & = & \int_{-\infty}^0 \frac{\phi(y)}{\pi y^2(1+\frac{1}{y^2})} dy + \int_0^{\infty} \frac{\phi(y)}{\pi y^2(1+\frac{1}{y^2})} dy \\  & = & \int_{-\infty}^{\infty}  \frac{\phi(y)}{\pi (1+y^2)} dy \end{eqnarray*}$$
...
1. Si $\phi : \mathbb{R} \to \mathbb{R}_+$ est borélienne on a (on effectue le changement de variables $z=\frac{1}{1+x^2}$ à la toisième ligne ci-dessous) 
   ...
   $$\begin{eqnarray*} \mathbb{E}[\phi(Z)] & = & \mathbb{E}[\phi(\frac{1}{1+X^2})] \\ & = & \int_{-\infty}^{0} \frac{\phi(\frac{1}{1+x^2})}{\pi(1+x^2)} dx  + \int_{0}^{\infty} \frac{\phi(\frac{1}{1+x^2})}{\pi(1+x^2)} dx \\ & = & 2 \int_{0}^1 \frac{z \phi(z)}{\pi} \frac{1}{2 z^2\sqrt{\frac{1}{z}-1}}  \\  & = & \int_{0}^{1} \frac{\phi(z)}{\pi\sqrt{z(1-z)}} \end{eqnarray*}$$
...
```
 ...
 (calcul d'une loi conditionnelle discrète) 
...
$$
\begin{eqnarray*}
 \int_{\mathbb{R}^2} f(x,y) dx dy  & = & \frac{1}{4} \int_{\mathbb{R}^2}   \frac{4}{\pi \sigma^2} \exp\left(-\frac{x^2+y^2}{\sigma^2}\right) \\ 
...
$$\begin{eqnarray*}
 \mathbb{E}[\psi(X-Y, X+Y)] & = & \int_{\mathbb{R}_+^2}  \psi(u,v) \frac{2}{\pi \sigma^2} \exp\left(-\frac{u^2 + v^2}{2\sigma^2}\right)  du dv \\ 
...
$$
\begin{eqnarray*}
 \mathrm{Var}(Z) & = & \sum_{k=1}^n \sum_{\ell=1}^n \alpha_k \alpha_{\ell} \mathrm{Cov}(X_k, X_{\ell}) \\ & = & \alpha^T A \alpha. 
\end{eqnarray*}
$$
...
$$\begin{eqnarray*} \mathbb{E}[\phi(X+Y)] & = & \int_{[0,1]^2} \phi(x+y) \mathrm{d}x \mathrm{d}y \end{eqnarray*}$$
...
$$\begin{eqnarray*}\mathbb{E}[\phi(X+Y)] & = & \int_{0}^2 ds \phi(s) \int_{\max(s-1,0)}^{\min(s,1)} \mathrm{d}t\\ & = &  \int_0^2 \phi(s) \min(s, 1-s) \mathrm{d}s \end{eqnarray*}$$
...
$$\begin{eqnarray*} \mathbb{E}[\phi(U,V)] & = & \int_{0}^1 \int_0^1 dx dy \phi \left(\sqrt{-2\ln(x)} \cos(2\pi y), \sqrt{-2\ln(x)} \sin(2\pi y)\right) \\ & = & \int_{\mathbb{R}^2} du dv \phi(u,v) \frac{1}{2\pi} \exp(-u^2/2) \exp(-v^2/2), \end{eqnarray*}$$
...
$$\begin{eqnarray*} \mathbb{E}[\phi(X,Z)] & = & \frac{1}{2\pi} \int_{(\mathbb{R}^*)^2} \phi(x, \frac{y}{x}) \exp(-x^2/2-y^2/2) dx dy \\ & = & \frac{1}{2\pi} \int_{(\mathbb{R}^*)^2} \phi(x, z) |x| \exp\left(-\frac{x^2}{2} (1+z^2)\right) dx \mathrm{d}z   \end{eqnarray*}$$
...
$$\begin{eqnarray*} f_Z(z) & = & 2 \int_{\mathbb{R}_+} \frac{1}{2\pi}  x z^2 \exp\left(-\frac{x^2}{2} (1+z^2)\right) dx = \frac{1}{\pi} \frac{1}{1+z^2}, \end{eqnarray*}$$
...
1. Quitte à considérer les $\mathcal{C}^1$-difféomorphisme
    ...
    $$\begin{eqnarray*}\mathbb{E}[\phi(U,V)]   =  2 \int_{\mathbb{R} \times \mathbb{R}_+} \phi(u,v) \frac{\exp\le(-\frac{v}{2}\right)}{4 \pi \sqrt{u-v^2}} \mathbf{1}_{\{v \ge u^2\}}du dv \end{eqnarray*}$$
 
    et on conclut que $(U,V)$ possède la densité $f_{(U,V)}$ telle que pour tout $(u,v) \in \mathbb{R}^2$, 

    $$f_{(U,V)}(u,v) = \frac{\exp\left(-\frac{v}{2}\right)}{2 \pi \sqrt{u-v^2}} \mathbf{1}_{\{v \ge u^2\}}.$$  
...
1. On a pour $\phi : \mathbb{R}^2 \to \mathbb{R}_+$ borélienne 
    ...
    $$\begin{eqnarray*} \mathbb{E}[\phi(U,V)] & = &  \int_{\mathbb{R}_+} \phi( [x], x-[x])  \exp(-x) dx \\ & = & \sum_{n \ge 0} \int_n^{n+1} \phi(n, x-n) \exp(-x) dx \\ & = & \sum_{n \ge 0} \int_0^1 \phi(n,v) \exp(-n) \exp(-v) du \end{eqnarray*}$$
 
    Si $g : \mathbb{R} \to \mathbb{R}_+$, $h : \mathbb{R} \to \mathbb{R}_+$ boréliennes, quitte à considérer $\phi(u,v) = g(u) h(v)$ on obtient 

    $$\begin{eqnarray*}\mathbb{E}[g(U) h(V)] & = & \sum_{n \ge 0} g(n) \exp(-1)^n (1-\exp(-1)) \int_0^1 h(v) \frac{\exp(-v)}{1-\exp(-1)} du \\  & = & \mathbb{E}[g(U)] \mathbb{E}[h(V)]  \end{eqnarray*}$$
...
1. Par hypothèse, la loi de $(X_1,X_2)$ est uniforme sur $\{1,2,3\}^2$, et on déduit, quitte à disjoindre les cas, que  
    ...
    $$\begin{eqnarray*} && \mathbb{P}((U,V) =(1,1)) = \mathbb{P}(X_1=1, X_2=1) = 1/9 \\  && \mathbb{P}((U,V)=(1,2)) = \mathbb{P}(X_1=1,X_2=2) + \mathbb{P}(X_1=2,X_2=1) = 2/9 \\ && \mathbb{P}((U,V) = (1,3)) =  \mathbb{P}(X_1=1,X_2=3) + \mathbb{P}(X_1=3,X_2=1) = 2/9 \\ && \mathbb{P}((U,V) = (2,2)) = \mathbb{P}(X_1=2, X_2=2) = 1/9 \\  && \mathbb{P}((U,V)=(2,3)) = \mathbb{P}(X_1=2,X_2=3) + \mathbb{P}(X_1=3,X_2=2) = 2/9 \\ && \mathbb{P}((U,V)=(3,3)) = \mathbb{P}(X_1=3,X_2=3) = 1/9 \end{eqnarray*}$$
 

    La même disjonction de cas conduit à la loi de $(V,S)$ : 

    $$\begin{eqnarray*} && \mathbb{P}(V=1,S=2) = 1/9 \\  && \mathbb{P}(V=2,S=3) = 2/9 \\  && \mathbb{P}(V=3,S=4) = 2/9 \\ && \mathbb{P}(V=2,S=4) = 1/9 \\ && \mathbb{P}(V=3, S=5) = 2/9 \\  && \mathbb{P}(V=3, S=6) = 1/9  \end{eqnarray*}$$
  

1. On en déduit (on peut également faire un raisonnement direct)  

   $$\begin{eqnarray*} && \mathbb{P}(U=1) = 5/9, \ \mathbb{P}(U=2)=1/3, \ \mathbb{P}(U=3)=1/9 \\  && \mathbb{P}(V=1) = 1/9, \ \mathbb{P}(V=2) = 1/3,\ \mathbb{P}(V=3)=5/9 \\ && \mathbb{P}(S=2) = \mathbb{P}(S=6) = 1/9, \ \mathbb{P}(S=3)=\mathbb{P}(S=5) = 2/9. \ \mathbb{P}(S=4)=1/3 \end{eqnarray*}$$
...
1. Quelle est la loi de $Z= \varepsilon$ 
...
1. Soit $\phi : \mathbb{R} \to \mathbb{R}_+$ borélienne
    ...
    $$\begin{eqnarray*}\mathbb{E}[\phi(\varepsilon] & = & \mathbb{E}[\phi(Y) \mathbf{1}_{\{\varepsilon}] + \mathbb{E}[\phi(-Y) \mathbf{1}_{\{\varepsilon}}]  \\ & = & p \mathbb{E}[\phi(Y)] + (1-p) \mathbb{E}[\phi(-Y)] \\ & = & p \int_{\mathbb{R}} \phi(z) \frac{1}{\sqrt{2\pi}} \exp(-z^2/2) \mathrm{d}z + (1-p) \int_{\mathbb{R}} \phi(-z) \frac{1}{\sqrt{2\pi}} \exp(-z^2/2) \mathrm{d}z  \\ & = & \int_{\mathbb{R}} \phi(z) \frac{1}{\sqrt{2\pi}} \exp(-z^2/2) \mathrm{d}z \end{eqnarray*}$$
...
$$\begin{eqnarray*} \Phi_X(t) & = & \exp(it^Tm + PKP^T) \\  &  = & \exp\bigg(i(t_1+t_2) - (t_1^2+t_2^2+t_3^2+t_1 t_2+t_1 t_3+2t_2 t_3)\bigg) \end{eqnarray*}$$
 
1. On applique le résultat général avec $P = (2 \ \ 1 \ \ 1)$ puis avec $P=(4 \ -2 \ \ 1)$, et enfn avec $P=(0 \ \ 1 \ -1)$ pour obtenir 
...
1. On a $\begin{pmatrix} X \\ Y \end{pmatrix} \sim \mathcal{N} \left( \mu=\begin{pmatrix} 1 \\ 1 \end{pmatrix}, \Sigma = \begin{pmatrix} 2 & 1 \\ 1 & 2 \end{pmatrix}\right)$. On a  $\det(\Sigma)=3$, et est $\Sigma^{-1} = \frac{1}{3} \begin{pmatrix} 2 & -1 \\ -1 & 2 \end{pmatrix}$, et donc ce vecteur possède la densité $f$ sur $\mathbb{R}^2$, où  pour $x \in \mathbb{R}^2$ de coordonnées $(x_1,x_2)$,  

    $$\begin{eqnarray*} f(x) & = & \frac{1}{2\pi\sqrt{3}} \exp(-\frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu) \\ & = & \frac{1}{2 \pi \sqrt{3}} \exp\left(-\frac{1}{3} ((x_1-1)^2 + (x_2-1)^2 - (x_1-1)(x_2-1)) \right) \end{eqnarray*}$$
 
1. On obtient ici que  
...
1. On a $\det(M) = 1-\rho^2 \ne 0$, $M^{-1} =  \frac{1}{1-\rho^2}  \begin{pmatrix} 1 & - \rho \\ -\rho & 1 \end{pmatrix}$, et donc $\begin{pmatrix} X \\ Y \end{pmatrix}$ possède la densité $f_{(X,Y)}$ où, pour $ \begin{pmatrix} x \\ y \end{pmatrix} \in \mathbb{R}^2$,   
    ...
    $$\begin{eqnarray*} f_{(X,Y)}(x) & = & \frac{1}{2\pi \sqrt{1-\rho^2}} \exp\left(-\frac{1}{2} \begin{pmatrix} x \\ y \end{pmatrix}^T M^{-1} \begin{pmatrix} x \\ y \end{pmatrix}  \right)\\  & = & \frac{1}{2\pi \sigma} \exp\left(- \frac{1}{2 \sigma^2} \left(x^2 + y^2 - 2 \rho x y \right) \right) \end{eqnarray*}$$
  
1. $X \sim \mathcal{N}(0,1)$ donc $f_X(x) = \frac{1}{\sqrt{2\pi}} \exp(-x^2/2), x \in \mathbb{R}$. On a donc, pour $y \in \mathbb{R}$  

    $$\begin{eqnarray*}  g_x(y) & = & \frac{1}{\sigma \sqrt{2 \pi}} \exp\left( \frac{1}{2(1-\rho^2)} (x^2+y^2 - 2\rho x y) + \frac{1}{2} x^2 \right) \\  & = & \frac{1}{\sigma \sqrt{2\pi}} \exp\left( \frac{1}{2(1-\rho^2)} \left( - \frac{1}{2} (\rho^2 x^2 - 2 \rho x y + y ^2) \right)\right) \\ & = &  \frac{1}{\sigma \sqrt{2\pi}} \exp\left( \frac{1}{2(1-\rho^2)} \left( - \frac{1}{2} (y-\rho x)^2 \right) \right) \end{eqnarray*}$$
...
Variable aléatoire. Image d'une variable aléatoire
: Il est important de noter que la définition de "loi" d'une variable aléatoire réelle est étroitement liée à la notion de mesure image. Par exemple, supposons $(\Omega, \mathcal{F}, \mathbb{P})$ est un espace probabilisé, et
