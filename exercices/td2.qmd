...
::: {#exr-1  name="Espérance conditionnelle/tribu atomique"}
...
::: {#exr-2  name="Conditionnement continu"}
...
2. On fixe $t>0$ et on suppose $X_t \sim \mathrm{Poisson}(t)$. Que vaut 
...
2.  Conditionnellement à $\{N_t=2\}$, quelle est la loi du vecteur $(\mathbf{e}_1; \mathbf{e}_2)$?  
...
2. Déterminer $\alpha$ un réel tel que
...
2. Déterminer un réel $\beta$ tels que $Z=\beta X_1 + X_3$ est
...
2. Vérifier que pour une fonction $f : \mathbb{R} \to \mathbb{C}$ telle que $f(Y) \in \mathbb{L}^1$, on a  
...
2. Déduire que si $t\in \mathbb{R}$ 
...
1. On a grâce à la propriété de tour et la question précédente  
...
2. Montrer que $(X,Y)$ possède une densité jointe $g$ que l'on déterminera.  
3. Montrer que $(U,Y)$ possède la densité jointe 
...
2.  Trouver $a \in \mathbb{R}$ tel que $X_1$ et $Y=X_2-a X_1$ soient indépendantes. Calculer $\mathrm{Var}(Y)$ et en déduire la loi de $(X_1,Y)$. 
3.  Trouver la loi conditionnelle de $X_2$ sachant $X_1$. 
...
::: {#exr-17  name="Combinaison linéaire de gaussiennes"}
...
::: {#exr-18  name="Loi jointe à densité"}
...
::: {#exr-19  name="Exponentielles conditionnées"}
...
::: {#exr-20  name="Gaussiennes corrélées"}
...
::: {#exr-21  name="Gaussiennes corrélées (2)"}
...
::: {#exr-24  name="Maxima d'exponentielles"}
...
1. Calculer  $\mathbb{E}[\max(X_1,X_2) \mid X_1]$. 
